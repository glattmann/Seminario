...
import sistemaexpedientes.persistencia.ContribuyenteDAOJson;
...
public class VentanaContribuyente extends JFrame {
    ...
    private List<Contribuyente> listaContribuyentes = ContribuyenteDAOJson.leer(); // Lista general de todos los contribuyentes guardados en el JSON
    ...
    private void inicializarComponentes() {
        ...
        btnCargarAExpediente.addActionListener(e -> {
            if (ventanaExpediente != null) {
                Contribuyente c = obtenerContribuyenteDesdeCampos();
                ventanaExpediente.setContribuyenteSeleccionado(c);
                dispose();
            }
        });
    ...
    // Guarda el contribuyente nuevo o actualizado en el JSON
    private void guardarContribuyente() {
        Contribuyente c = obtenerContribuyenteDesdeCampos();
        if (contribuyenteActual == null) {
            listaContribuyentes.add(c); // nuevo
        } else {
            listaContribuyentes.remove(contribuyenteActual); // actualización
            listaContribuyentes.add(c);
        }
        ContribuyenteDAOJson.guardar(listaContribuyentes);
